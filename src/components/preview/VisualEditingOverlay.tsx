...
// the page never did. An MPA adapter instead: any push/replace to a different URL
// is a REAL load. Module-level so the overlay never resubscribes on re-render.
...
//     attributes from src/lib/preview-edit-attr.ts into in-canvas array controls
//     (insert / duplicate / remove / drag), with no extra props.
...
//        listen API into tiny "change" signals). Event-driven on purpose; never
//        reintroduce an interval poll here.
...
export default function VisualEditingOverlay({ pageId }: Props) {
  ...
  // reconnect a fresh non-CDN Sanity request on the API quota. Hidden closes it;
  // visible again reopens and does ONE catch-up refetch.
  useEffect(() => {
  ...
  const refresh = useCallback(
    (payload: HistoryRefresh): false | Promise<void> => {
      ...
      // when a shared doc changed (siteSettings, gallery items, categories,
      // pricing tiers, thread colors, fonts, FAQ items): anything that is not
      // some OTHER page singleton can appear on the page being previewed.
      const id = payload.document._id.replace(/^drafts\./, '');
      const isThisPage = id === pageId;
      const isOtherPageSingleton = /Page$/.test(payload.document._type) && !isThisPage;
      return isThisPage || !isOtherPageSingleton ? softRefresh() : false;
    },
    [pageId, softRefresh],
  );
