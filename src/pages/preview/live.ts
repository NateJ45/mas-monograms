...
// =============================================================================
// The Presentation preview shows DRAFT content, so listening for edits needs the
// Sanity token, which must never reach the browser. This endpoint holds the token
// server-side: it opens ONE long-lived connection to Sanity's listen API (SSE
// mutation events, GROQ-filtered) and forwards a tiny "change" signal to the
// preview overlay, which soft-refetches the page (VisualEditingOverlay).
//
...
// or when any doc that is not a page singleton changes. On this site every page
// is a singleton whose content is its own fields, but the pages also render
// COLLECTION docs (galleryItem, itemCategory, pricingTier, clearanceItem,
// threadColor, font, faqItem) and siteSettings, any of which can appear on the
// page being previewed. So the filter stays silent only for edits to some OTHER
// page singleton.
//
...
// The page singleton types. An edit to one of these matters only when it IS the
// document being previewed; everything else on the site can show up anywhere.
// Keep in step with SINGLETON_BY_PATH in ./[...slug].astro.
...
const LISTEN_QUERY = `*[_id in [$pageId, $draftId] || !(_type in ${JSON.stringify(PAGE_SINGLETONS)})]`;
...
export const GET: APIRoute = async ({ cookies, url, request }) => {
  ...
  // No draft content flows through here, but there is no reason to let anonymous
  // visitors hold open upstream Sanity connections either.
  ...
  // Doc id of the page being previewed (a singleton id like "homePage"). Ids are
  // passed to Sanity as JSON-encoded GROQ params, never spliced into the query;
  // the shape check just keeps garbage out.
  ...
  upstream.searchParams.set('tag', 'mas.preview-live');
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      // request (metered on the API quota), so this is deliberately unhurried:
      // 15s turns a dropped connection into an occasional request instead of a
      // reconnect storm. The manual refresh button still works during the gap,
      // and the overlay does a catch-up refetch on reconnect, so no edit is lost.
      send('retry: 15000\n\n');
      // Comment heartbeat so proxies and Cloudflare never see an idle connection
      ...
      try {
        ...
        const token = e.SANITY_TOKEN || e.SANITY_API_READ_TOKEN;
        ...
        // separated by a blank line; normalize CRLF on the concatenated buffer so
