#!/usr/bin/env node
// PORTED 2026-08-27 from ncs-astro-sanity-starter (PORTS.md card 3). NOT marked
// PORTABLE on purpose: the card records this harness as a PATTERN, not an
// identical canonical file — a site's normalizer grows site-local rules for that
// site's proven nondeterminism (WCP added Instagram rules), and the moment one
// lands here a byte-exact sync-check would report drift forever. Keep this file
// out of sync-check and reconcile it by reading card 3 instead. As of this port
// the body is unmodified from the starter's copy; any site-local rule added
// below must say WHY, with the evidence that the value really is build-varying.
/**
