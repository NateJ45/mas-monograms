...
Run: `grep -rn "font-script" src` after deleting it. As written, this step assumed no live component still used the utility — that assumption turned out to be wrong (see the note below) and the instruction actually followed during implementation was: report any remaining usages but do NOT fix them in this task. The original "fix that usage now" framing here is superseded.

**Correction found during implementation:** `Hero.astro`, `SectionHeading.astro`, and `FinalCta.astro` all actively render a `.font-script` span for a headline accent word, not just the logo. Removing the utility silently degrades their accent-word styling (falls back to the body font, no error) rather than breaking the build. Properly fixing this means redesigning how these three components present a headline accent (per the design spec: "accent words use a solid pine-teal or rust span instead" of script italics) — a component-level content/design decision, not a CSS-token change, so it does NOT belong in this CSS-only task. It's logged as an explicit follow-up for the page-by-page redesign plan (Plan 3) in "What this plan does NOT cover" below. Do not fix `Hero.astro`, `SectionHeading.astro`, or `FinalCta.astro` in this task — just confirm the grep findings match this list and report them.
...
- **Redesigning the headline-accent treatment in `Hero.astro`, `SectionHeading.astro`, and `FinalCta.astro`.** Task 6 deleted the `font-script` CSS utility (no script font in Thread Ledger), but these three live components still render a `.font-script` span for an accent word — discovered during Task 6's implementation, after this plan was originally written. The class removal doesn't break the build (it's a silent style fallback, not an error), but it does mean these components' accent-word styling visually degrades to the body font until they're redesigned to use a solid pine-teal/rust span instead, per the design spec. This is real, scoped, page-by-page redesign work (Plan 3's job), not a foundation-layer fix — but flag it explicitly when Plan 3 starts so it isn't missed.
- **De-duplicating the Logo SVG markup between `Logo.astro` and `MobileNav.tsx`.** Task 8 discovered `Header.astro` was passing logo-asset props to `MobileNav.tsx` (a React island) that broke when the raster logo was removed; the fix ported `Logo.astro`'s wordmark SVG into `MobileNav.tsx` as inline JSX (Astro components can't be imported into React islands directly), since `Logo.astro` wasn't in either plan's original file list. The two copies are color/geometry-identical today but have no shared source of truth — a future palette or logo-shape change must remember to update both. Low urgency, not a launch blocker; consider a shared TS constants module (hex values only, since SVG presentation attributes can't read CSS custom properties) the next time either file is touched.
